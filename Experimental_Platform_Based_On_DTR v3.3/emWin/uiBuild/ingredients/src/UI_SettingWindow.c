...
#define ID_BUTTON_DEVID						(GUI_ID_USER + 0x21)
...
static const GUI_WIDGET_CREATE_INFO _aDialogCreate[] = {
	...
	{ BUTTON_CreateIndirect, "Button", ID_BUTTON_DEVID, 220, 300, 120, 40, 0, 0x0, 0 },
...
static void pushButtonInitDialog(void)
{
	...
	BUTTON_SetFont(hDevIdButton,  &YaHei_36_Font);
	BUTTON_SetTextAlign(hDevIdButton, GUI_TA_HCENTER | GUI_TA_VCENTER);
	BUTTON_SetTextColor(hDevIdButton, BUTTON_CI_DISABLED, GUI_BLACK);
	BUTTON_SetTextColor(hDevIdButton, BUTTON_CI_PRESSED, GUI_BLACK);
	BUTTON_SetTextColor(hDevIdButton, BUTTON_CI_UNPRESSED, GUI_BLACK);
	BUTTON_SetText(hDevIdButton, "上传ID");
	WM_SetCallback(hDevIdButton, _cbPushButtonText);
...
static void _cbDialog(WM_MESSAGE * pMsg)
{
	...
	case WM_INIT_DIALOG:
		...
		hDevIdButton = WM_GetDialogItem(pMsg->hWin, ID_BUTTON_DEVID);
		WM_HideWindow(hDevIdButton);
	...
	case WM_NOTIFY_PARENT:
		...
		case ID_LISTVIEW_CONNECT_LIST:
			...
			case WM_NOTIFICATION_RELEASED:
				...
				BUTTON_GetText(hScanButton, (char *)p, sizeof(p));
				if(strstr("刷新列表", (char *)p)) {
					WM_SetUserData(hFullKeyboard, &hConnectEdit, sizeof(WM_HWIN));
