...
for  the  purposes  of  createing  libraries  for  ARM7, ARM9, Cortex-M
...
/*********************************************************************
*
*       Optimization enable for GUI
*/
#define GUI_Optimization 1
...
#define yPhys y
    ...
    GUI_USE_PARA(PixelIndex);
    {
        ...
        // TBD by customer...
        //
       lcd.put_pixel_gui(x, y, PixelIndex);
    }
...
{
    static LCD_PIXELINDEX PixelIndex;
    //
    // Convert logical into physical coordinates (Dep. on LCDConf.h)
    //
#if (LCD_MIRROR_X == 1) || (LCD_MIRROR_Y == 1) || (LCD_SWAP_XY == 1)
...
#define yPhys y
    ...
    GUI_USE_PARA(y);
    {
        ...
        // TBD by customer...
        //
        PixelIndex = lcd.get_pixel_gui(xPhys, yPhys);
...
#if GUI_Optimization
   lcd.draw_H_line(x0, y, x1, PixelIndex);
#else
    for (x = x0; x <= x1; x++)
    {
...
#if GUI_Optimization
        PixelIndex = LCD__GetColorIndex();
        lcd.fill_rect(x0, y0, x1, y1, PixelIndex);
#else
        for (; y0 <= y1; y0++)
        {
...
#if GUI_Optimization
    PixelIndex = LCD__GetColorIndex();
    lcd.draw_V_line(x, y0, y1, PixelIndex);
#else
...
{
#if GUI_Optimization
    lcd.draw_H_colorLine(x, y, xsize, (uint16_t *)p);
#else
    for (; xsize > 0; xsize--, x++, p++)
    {
        _SetPixelIndex(pDevice, x, y, *p);
