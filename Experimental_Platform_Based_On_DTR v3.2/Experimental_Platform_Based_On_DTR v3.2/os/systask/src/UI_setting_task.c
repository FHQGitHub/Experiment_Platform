...
static void open()
{
	WM_BringToTop(settingwin.hwin);
	WM_ShowWin(settingwin.hwin);
}
	...
	switch(ui_state.settingwin_state)
	{
		...
		case STATE_SETTING_WIFI_SCAN:
			...
			if(!WM_IsVisible(WM_GetDialogItem(ui.settingwin->hwin,ID_LISTVIEW_0)))
			{
				...
				TEXT_SetText(WM_GetDialogItem(settingwin.hwin, ID_TEXT_0), "无线网络扫描中 ");
		...
		case STATE_SETTING_VOICE_VOLUME:
			...
			sprintf((char*)VolumeString,"[v%c]sound102",NowData.VolumeLevel+'0');
		...
		case STATE_SETTING_ENTER_ADMIN:
		...
		case STATE_SETTING_WAITING:
			if(WM_IsVisible(ui.wifi_connectwin->hwin))	
			{
			...
			if(WM_IsCompletelyCovered(ui.wifi_connectwin->hwin))
			{
