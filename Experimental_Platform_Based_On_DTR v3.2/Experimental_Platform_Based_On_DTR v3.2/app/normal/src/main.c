...
enum flag_t wifiStateCheckFlag = flag_reset;
...
//	usr_c322_init();
	...
	while(1) {
		...
		if(xResult == pdPASS) {
			...
			switch (notify.mqtt_notify.event) {
				...
				case mqtt_restart:
					...
					else if(WIFI_TCP_ERR == wifiRetVal)
		...
		if(flag_set == wifiStateChangeFlag || flag_set == wifiStateCheckFlag) {
			if(notify.setting.event != wifi_connect) {
				if(1 == Nlink) {
