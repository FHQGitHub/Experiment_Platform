		...
		switch(notify.setting.event) {
			case wifi_connect:
				switch(notify.setting.status) {
					case WIFI_OK:
						memset(&notify.setting, 0 , sizeof(notify.setting));
						notify.setting.event = wifi_ping_check;
						if(errQUEUE_FULL == xQueueSend(xQueueWifi, (const void *)&notify, 50)) 
							notify_show("检查服务器连接失败", "请稍后");
						notify_show("无线网络", "连接成功");
					break;
					...
					case WIFI_ERR:
						...
						notify_show((char *)wifi_data.name, "WIFI未连接");
					break;
			...
			case wifi_tcp_check:
				switch(notify.setting.status) {
					case WIFI_OK:
						...
						memset(&notify, 0 , sizeof(notify));
						notify.http_notify.event = http_sync_time;
