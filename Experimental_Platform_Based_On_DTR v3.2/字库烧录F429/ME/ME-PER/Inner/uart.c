...
//串口发送缓存区 	
__align(8) u8 UART5_TX_BUF[UART5_MAX_SEND_LEN]; //发送缓冲,最大UART53_MAX_SEND_LEN字节
#ifdef UART5_RX_EN   								//如果使能了接收   	  
//串口接收缓存区 	
u8 UART5_RX_BUF[UART5_MAX_RECV_LEN]; 				//接收缓冲,最大UART53_MAX_RECV_LEN个字节.
...
void UART5_IRQHandler(void)
{
	...
	{	 
		...
		{ 
			if(times<UART5_MAX_RECV_LEN)		//还可以接收数据
			{
				TIM_SetCounter(TIM7,0);//计数器清空 重新计时 确定每两个字符间隔时间是否大于100ms			 
				if(UART5_RX_STA==0)	//从收到第一个数据开始计时
				{
					UART5_RX_STA|=1<<14; //标记发起者
					times=0;
					TIM_Cmd(TIM7, ENABLE);  //使能定时器7 
				}
				UART5_RX_BUF[times++]=res;		//记录接收到的值
				UART5_RX_STA++;			
			}
			else 
			{
...
{  
	...
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_UART5,ENABLE);//使能UART53时钟

 	USART_DeInit(UART5);  //复位串口3
	
	GPIO_PinAFConfig(GPIOC,GPIO_PinSource12,GPIO_AF_UART5); //GPIOB11复用为UART53
	GPIO_PinAFConfig(GPIOD,GPIO_PinSource2,GPIO_AF_UART5); //GPIOB10复用为UART53	
...
//串口3,printf 函数
//确保一次发送数据不超过UART53_MAX_SEND_LEN字节
...
void UART4_IRQHandler(void)
{
	...
	{	 
		...
		{ 
			if(times<UART4_MAX_RECV_LEN)		//还可以接收数据
			{
				TIM_SetCounter(TIM7,0);//计数器清空 重新计时 确定每两个字符间隔时间是否大于100ms			 
				if(UART4_RX_STA==0)	//从收到第一个数据开始计时
				{
					UART4_RX_STA|=1<<14; //标记发起者
					times=0;
					TIM_Cmd(TIM7, ENABLE);  //使能定时器7 
				}
				UART4_RX_BUF[times++]=res;		//记录接收到的值
				UART4_RX_STA++;			
			}
			else 
			{
...
{  
	...
	GPIO_PinAFConfig(GPIOA,GPIO_PinSource0,GPIO_AF_UART4); //GPIOB11复用为USART3
	GPIO_PinAFConfig(GPIOA,GPIO_PinSource1,GPIO_AF_UART4); //GPIOB10复用为USART3	
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_1; //GPIOB11和GPIOB10初始化
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;//复用功能
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;	//速度50MHz
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP; //推挽复用输出
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP; //上拉
...
void USART3_IRQHandler(void)
{
	...
	{	 
		...
		{ 
			if(times<USART3_MAX_RECV_LEN)		//还可以接收数据
			{
				TIM_SetCounter(TIM7,0);//计数器清空 重新计时 确定每两个字符间隔时间是否大于100ms			 
				if(USART3_RX_STA==0)	//从收到第一个数据开始计时
				{
					USART3_RX_STA|=1<<14; //标记发起者
					times=0;
					TIM_Cmd(TIM7, ENABLE);  //使能定时器7 
				}
				USART3_RX_BUF[times++]=res;		//记录接收到的值
				USART3_RX_STA++;			
			}
			else 
			{
