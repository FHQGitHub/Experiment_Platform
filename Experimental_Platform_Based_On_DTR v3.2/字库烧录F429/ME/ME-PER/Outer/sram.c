...
void FMC_SRAM_Init(void)
{
	...
	FMC_ReadWriteTimingStructure.FMC_AddressSetupTime = 0X00; //地址建立时间0个HCLK 0ns
	FMC_ReadWriteTimingStructure.FMC_AddressHoldTime = 0x00;  //地址保持时间,模式A未用到
	FMC_ReadWriteTimingStructure.FMC_DataSetupTime = 0x09;    //数据保持时间,9个HCLK，6*9=54ns
	...
	FMC_ReadWriteTimingStructure.FMC_AccessMode = FMC_AccessMode_A; //模式A
	
	FMC_NORSRAMInitStructure.FMC_Bank = FMC_Bank1_NORSRAM3;  //NOR/SRAM的Bank3
	FMC_NORSRAMInitStructure.FMC_DataAddressMux = FMC_DataAddressMux_Disable; //数据和地址线不复用
	FMC_NORSRAMInitStructure.FMC_MemoryType = FMC_MemoryType_SRAM; //SRAM存储模式
	FMC_NORSRAMInitStructure.FMC_MemoryDataWidth = FMC_NORSRAM_MemoryDataWidth_16b; //16位数据宽度
	FMC_NORSRAMInitStructure.FMC_BurstAccessMode = FMC_BurstAccessMode_Disable; //FLASH使用的,SRAM未使用
	FMC_NORSRAMInitStructure.FMC_AsynchronousWait = FMC_AsynchronousWait_Disable; //是否使能同步传输模式下的等待信号,此处未用到
	FMC_NORSRAMInitStructure.FMC_WaitSignalPolarity = FMC_WaitSignalPolarity_Low; //等待信号的极性,仅在突发模式访问下有用
	FMC_NORSRAMInitStructure.FMC_WrapMode = FMC_WrapMode_Disable;  //是否使能环路突发模式,此处未用到
	FMC_NORSRAMInitStructure.FMC_WaitSignalActive = FMC_WaitSignalActive_BeforeWaitState; //存储器是在等待周期之前的一个时钟周期还是等待周期期间使能NWAIT
	FMC_NORSRAMInitStructure.FMC_WriteOperation = FMC_WriteOperation_Enable; //存储器写使能
	FMC_NORSRAMInitStructure.FMC_WaitSignal = FMC_WaitSignal_Disable;   //等待使能位,此处未用到
	FMC_NORSRAMInitStructure.FMC_ExtendedMode = FMC_ExtendedMode_Disable; //读写使用相同的时序
	FMC_NORSRAMInitStructure.FMC_WriteBurst = FMC_WriteBurst_Disable;  //异步传输期间的等待信号
	...
	FMC_NORSRAMInit(&FMC_NORSRAMInitStructure);  //FMC_SRAM初始化
	
	FMC_NORSRAMCmd(FMC_Bank1_NORSRAM3,ENABLE);  //使能NOR/SRAM功能
}
...
{
	FMC_SRAM_WriteBuffer(&data,addr,1);//写入一个字节
}
