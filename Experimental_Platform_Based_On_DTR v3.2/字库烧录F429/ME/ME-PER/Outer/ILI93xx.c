...
{ 	
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOC|RCC_AHB1Periph_GPIOD|RCC_AHB1Periph_GPIOE|RCC_AHB1Periph_GPIOF|RCC_AHB1Periph_GPIOG, ENABLE);//使能PD,PE,PF,PG时钟  
	RCC_AHB3PeriphClockCmd(RCC_AHB3Periph_FMC,ENABLE);//使能FMC时钟  
	
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;//PC6 推挽输出,控制复位
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;//普通输出模式
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;//推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//100MHz
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
	GPIO_Init(GPIOC, &GPIO_InitStructure);//初始化 //PC6 推挽输出,不复位
	GPIO_SetBits(GPIOC,GPIO_Pin_6);//初始化 //PC6 推挽输出,不复位

 
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11;//PD11 推挽输出,控制背光
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;//普通输出模式
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;//推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//100MHz
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
	GPIO_Init(GPIOD, &GPIO_InitStructure);//初始化 //PD11 推挽输出,控制背光
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_13;//PD13,FMC_A18
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;//复用输出
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;//推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100MHz
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
	GPIO_Init(GPIOD, &GPIO_InitStructure);//初始化  
	GPIO_SetBits(GPIOD, GPIO_Pin_13);//初始化  

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7;//PFD,FMC_NE1
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;//复用输出
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;//推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100MHz
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
	...
	GPIO_PinAFConfig(GPIOD,GPIO_PinSource13,GPIO_AF_FMC);//PD13,AF12
	...
	FMC_NORSRAMInitStructure.FMC_Bank = FMC_Bank1_NORSRAM1;//  这里我们使用NE1 ，也就对应BTCR[0],[1]。
	FMC_NORSRAMInitStructure.FMC_DataAddressMux = FMC_DataAddressMux_Disable; // 不复用数据地址
	FMC_NORSRAMInitStructure.FMC_MemoryType = FMC_MemoryType_NOR;// FMC_MemoryType_SRAM;  //SRAM   
	...
	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963)//如果是这三个IC,则设置WR时序为最快
	{
		...
		FMC_Bank1E->BWTR[0]|=3<<0;		//地址建立时间（ADDSET）为3个HCLK =18ns  	 
		FMC_Bank1E->BWTR[0]|=2<<8; 	//数据保存时间为6ns*3个HCLK=18ns
	}else if(lcddev.id==0X6804||lcddev.id==0XC505)	//6804/C505速度上不去,得降低
	{
		//重新配置写时序控制寄存器的时序   	 							    
		FMC_Bank1E->BWTR[0]&=~(0XF<<0);//地址建立时间（ADDSET）清零 	 
		FMC_Bank1E->BWTR[0]&=~(0XF<<8);//数据保存时间清零
		FMC_Bank1E->BWTR[0]|=10<<0;	//地址建立时间（ADDSET）为10个HCLK =60ns  	 
		FMC_Bank1E->BWTR[0]|=12<<8; 	//数据保存时间为6ns*13个HCLK=96ns
	}
	//printf("LCD ID:%x\r\n",lcddev.id); //打印LCD ID  
	...
	}else if(lcddev.id==0X9331)//OK |/|/|			 
	{
		...
		delay_xms(200); // Dis-charge capacitor power voltage
		LCD_WriteReg(0x0010, 0x1690); // SAP, BT[3:0], AP, DSTB, SLP, STB
		LCD_WriteReg(0x0011, 0x0227); // DC1[2:0], DC0[2:0], VC[2:0]
		delay_xms(50); // Delay 50ms
...
{          
 	...
 	}else
	{
		...
		for(i=sy;i<=ey;i++)
		{
			...
			LCD_WriteRAM_Prepare();     			//开始写入GRAM	  
			for(j=0;j<xlen;j++) LCD_RAM=color;	//显示颜色 	    
		}
	}	 
}  
//在指定区域内填充指定颜色块			 
...
{  
 	...
 	for(i=0;i<height;i++)
	{
		...
		LCD_WriteRAM_Prepare();     //开始写入GRAM
		for(j=0;j<width;j++) LCD_RAM=color[i*width+j];//写入数据 
	}		  
}  
//画线
//x1,y1:起点坐标
