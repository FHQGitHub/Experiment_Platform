	...
	if(!PcdRequest(PICC_REQALL,Card_Type))
	{
		if(!PcdAnticoll(Card_Serial_Number))
		{
			...
			if(!PcdSelect(Card_Serial_Number))
			{
				...
				if(!PcdAuthState(0x60, 0x01, DefaultKey, Card_Serial_Number))
				{
					printf("password correct\r\n");
					if(!PcdRead(1, Card_Read_Buf))
					{
						...
						value = Card_Read_Buf[0];
						value <<= 8;
						value |= Card_Read_Buf[1];
						
//						Card_Write_Buf[1] = 1;
//						PcdAuthState(0x60, 0x01, DefaultKey, Card_Serial_Number);
//						if(!PcdWrite(0x01,Card_Write_Buf))
//						{
//							printf("Write OK");
//						}
