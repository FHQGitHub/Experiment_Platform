...
u8 GT9147_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10 ;//PF10设置为上拉输入
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;//PB2设置为推挽输出
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10 ;//PF10设置为浮空输入
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;//输入模式
		...
		if(TouchStatus < 6) {
			...
			if(TouchStatus != 0) {
				for(i = 0; i < 6; i++) {
					...
					coordXY_raw[i].y = ((uint16_t)(xyCoord[1] & 0x0F) << 8) | (uint16_t)xyCoord[0];
					coordXY_raw[i].x = ((uint16_t)(xyCoord[3] & 0x0F) << 8) | (uint16_t)xyCoord[2];
